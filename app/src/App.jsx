...
export function GradeBadge({ text }) {
  ...
  return (
    ...
    >
      <span className="badge__glyph" aria-hidden="true">{meetsTarget ? '✓' : '△'}</span>
      <strong>{grade}</strong>
...
function generatingStatusText(status, hasRewrite) {
  if (status === 'generating') return 'Generating the plain-language rewrite…'
  if (status === 'error') return 'Generation failed. See the error message below.'
  if (status === 'done' && hasRewrite) return 'Rewrite ready. Review and correct it before approving.'
  return ''
}
      ...
      <div className="app">
        <header className="app__header">
          <div className="app__brand">
            <span className="app__logo" aria-hidden="true">NN</span>
            <div>
              <h1>Nurse Notes</h1>
              <p className="app__tagline">What the patient sees, once it's signed</p>
            </div>
          </div>
        </header>
    ...
    <div className="app">
      <header className="app__header">
        <div className="app__brand">
          ...
          <div>
            <h1>Nurse Notes</h1>
            <p className="app__tagline">Plain-language discharge summaries, signed by a nurse before release</p>
          </div>
        </div>
        <button
          className="link-btn"
      ...
      <div className="app__body">
        {showSettings && (
          <div className="settings" id="settings-panel">
            <label>
              LM Studio base URL
        ...
        {/* The gate banner — rendered as an open (unsigned) or closed (signed) chart clip. */}
        {isApproved ? (
          <div className="banner banner--approved">
            <span className="banner__icon" aria-hidden="true">✓</span>
            <div>
              <strong>Approved for release</strong>
              <span className="tag--wristband tag--signed" style={{ marginTop: 4 }}>
        ...
        <main className="panes">
          ...
          <section className="pane pane--plain">
            <div className="pane__head">
              <h2 id="plain-heading">Plain Language</h2>
            ...
            <p className="sr-only" role="status" aria-live="polite">
              {generatingStatusText(status, Boolean(rewrite.trim()))}
            </p>
            <textarea
              className="pane__text"
